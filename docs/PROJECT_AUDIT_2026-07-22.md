...
| `npm run test:unit` | пройдено | 12 файлов и 29 тестов пройдены; 1 файл/7 тестов правил ожидаемо пропущены без эмулятора. |
| `npm run test:rules` | пройдено | 7/7 тестов Firestore Rules через эмулятор. |
| `npm run build` | пройдено | Next.js 16.2.11; дополнительная проверка 10 server trace-манифестов не нашла секреты или файлы проекта. |
