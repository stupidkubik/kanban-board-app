...
- `retry-request`, `teeny-request`, `gaxios` и `uuid` — транзитивные moderate advisories в Google Cloud цепочках.
...
### P1 — `firebase-admin` 14 ломал production runtime Vercel — устранено

После обновления `firebase-admin` с 13.x до 14.2.0 production route `/` начал отвечать 500. Runtime logs показали `ERR_REQUIRE_ESM`: CommonJS loader Vercel Functions не смог загрузить ESM `jose@6`, который появился в цепочке `firebase-admin@14 -> jwks-rsa@4 -> jose@6`. Локальная сборка на Node.js 24 проходила, поэтому одного build-gate было недостаточно.

Решение: production build явно переведён на Webpack, а `firebase-admin` возвращён на совместимую ветку 13.x с `jwks-rsa@3 -> jose@4`. Деплой `dpl_4BB7cV11VXtMB8FHz6PSxvvkncTF` проверен на Vercel: `/` отвечает `307` на `/sign-in`, конечная страница — `200`. Повторный переход на 14.x допустим только отдельной миграцией через preview deployment, smoke-проверку server routes и просмотр runtime logs.
...
4. [x] Проверить миграцию `firebase-admin` 13 -> 14; выявлена несовместимость с Vercel runtime, поэтому production закреплён на совместимой ветке 13.x до upstream-исправления.
