...
Статус remediation: все локально исправимые P0/P1 и большинство P2/P3 закрыты отдельными проверенными коммитами. Fallback на credential-файл удалён, server traces проверяются автоматически, custom API получают App Check token, операции целостности перенесены в server transactions, rules coverage расширен, а основные read/performance риски ограничены. Production audit снижен до 9 записей (7 moderate, 2 high, 0 critical); оставшиеся цепочки Next.js и Firebase Admin требуют корректных upstream-релизов, а не предложенных npm несовместимых downgrade.
...
| `npm audit --omit=dev` | upstream-риск | Повторная проверка: 7 moderate и 2 high, без critical; совместимого auto-fix нет. |
...
| Cypress E2E | обновлён, не запускался | Учетные данные не настроены; suite теперь требует test-project opt-in и удаляет созданные доски в `afterEach`. |
...
Текущая экспозиция дополнительно ограничена: уязвимая Storage-цепочка Firebase Admin приложением не вызывается, а Next image optimizer принимает только аватары с allowlisted `lh3.googleusercontent.com`. Это не заменяет обновление зависимостей, но уменьшает доступную поверхность атаки до появления совместимых исправлений.
...
### P2 — несколько модулей стали точками концентрации логики — частично декомпозировано

Крупнейшие оставшиеся файлы после целевого разбиения:

- `lib/store/firestore-api.ts` — 697 строк;
- `features/cards/model/use-board-cards.ts` — 452;
- `lib/i18n.ts` — 560;
- `app/(auth)/sign-in/page.tsx` — 403;
- `features/board/ui/board-page.module.css` — 924.

Выполнено: DnD controller вынесен из `useBoardCards`; построение Firestore queries, snapshot normalization и limits вынесены из RTK orchestration в отдельные listener adapters; locale lifecycle уже объединён в provider hook. Оставшиеся крупные файлы в основном содержат связную orchestration, словарь переводов или стили. Дальнейшее дробление auth forms, card CRUD и CSS разумно выполнять вместе с их функциональными изменениями, а не отдельным рискованным mass-move.

### P2 — дублируются инфраструктурные обязанности — устранено

`toMillis`, email validation, locale lifecycle и извлечение сообщений ошибок теперь централизованы. Создание/отклонение invitations и восстановление column перенесены из UI/model hooks в соответствующие data operations; прямые Firestore writes остались только внутри data/store adapters и единого user-preference hook.

### P2 — нет error boundary и наблюдаемости — базовый уровень устранён

Добавлены route-level и root error boundaries с восстановлением через `reset`, понятным fallback UI и correlation id. Необработанные render errors журналируются единым структурированным событием без stack trace и содержимого пользовательских данных. Для production alerting следующим операционным шагом остаётся подключить внешний sink (например, Sentry/OTel) к `reportClientError`; это требует выбора сервиса и credentials.

### P3 — документация расходится с реализацией — устранено

`schema.md`, README и `docs/FUNCTIONAL_SPEC.md` синхронизированы с server API, атомарными операциями, редактируемым board language, query caps, rebalance, access/error states и актуальным rules coverage. `docs/FUNCTIONAL_SPEC.md` остаётся основой следующего рефакторинга.
...
4. [x] Virtualization не вводить до появления измеримой проблемы: текущий card cap 500 и основной bottleneck reads уже устранён.
...
3. [x] Добавить error boundaries, correlation id и structured logging; внешний telemetry sink остаётся deployment-решением.
...
- E2E не запускался из-за отсутствующих test-project credentials; write opt-in и cleanup теперь обязательны. Smoke не запускался, чтобы не изменять внешние данные.
