...
Статус документа: **as-is**, обновлено 27 июля 2026 года.
Назначение: единая точка входа для последующей доработки, тестирования и рефакторинга.
Источник истины: фактическая реализация в `app/`, `features/`, `lib/`,
`firestore.rules`; operational ограничения описаны в `OPERATIONS.md`.
...
- изменение роли принятого участника между editor/viewer владельцем доски;
...
- title, description, due date, несколько исполнителей и labels карточки;
- общий каталог labels на уровне доски;
...
Не входят в готовый пользовательский функционал:
...
- **Board language** — редактируемое поле metadata доски; оно не управляет персональным языком интерфейса.
...
| `POST /api/boards` | server route | атомарное создание board и owner profile |
| `GET /api/boards/[boardId]` | server route | различение not found/forbidden |
...
Создание session cookie ограничено десятью попытками в минуту на IP. Все ответы
получают явные CSP, anti-framing, MIME, referrer и permissions headers; CSP допускает
только необходимые Firebase/Google Auth/App Check endpoints.
...
- **Принять** — server transaction проверяет email/role, добавляет membership, создаёт отсутствующий member profile и удаляет invite атомарно.
...
До первого snapshot показывается loading state. Ошибка listener уточняется безопасным server access check и отображается как отдельное not-found, forbidden или retryable error состояние.
...
При достижении лимита 500 карточек UI предупреждает о неполной выборке и отключает редактирование отображаемого содержимого. Текущая эксплуатационная стратегия — разделить доску; cursor pagination не реализована, поскольку требует отдельной модели DnD между страницами.
...
- доступы owner/editor/viewer к board, columns и cards;
- ограничения формы данных и связей card-to-column/assignee-to-member;
- server-only rename, invite acceptance, remove/leave и удаление колонок;
- invite create/read/decline, memberProfiles и users.

Rules suite содержит 12 emulator-сценариев.

### 13.3 Cypress

Три сценария покрывают board/column/card CRUD, Undo, DnD, invites, labels,
нескольких assignees и realtime editor-to-viewer downgrade. Suite полностью
изолирован: launcher поднимает Auth и Firestore emulators для
`demo-kanban-e2e`, создаёт локальных пользователей, запускает Next.js и после
тестов независимо проверяет cleanup через Admin SDK. Внешние credentials и
cloud Firebase project не используются.
...
- Realtime content ограничен 500 cards, 100 columns и 100 member profiles; при card cap writes блокируются в UI.
