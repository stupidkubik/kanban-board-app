...
Известное ограничение: custom API ожидают App Check header при enforcement, но текущий client fetch его не отправляет.
...
- Listener errors для boards/columns/cards логируются или преобразуются в пустые данные.
- Специализированных retry/not-found/forbidden/offline states нет.
...
На главной странице locale:

1. Bootstrap-ится из `localStorage`.
2. Realtime-читается из `users/{uid}.preferredLocale`.
3. При пользовательском изменении записывается в user profile.

На `/sign-in` и `/boards/[boardId]` locale читается/пишется в `localStorage`. Board page не пишет profile напрямую; флаг `uiLocaleTouched` позволяет главной странице позже синхронизировать изменение.

### 8.2 Язык доски

`board.language` хранится как `ru/en`, выбирается при создании и показывается на карточке доски. Готового UI для изменения существующей board нет. Страница board не переключает copy по `board.language`.
